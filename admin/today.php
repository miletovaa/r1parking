...
* (We fetch only those rows where car is going to arrive or leave parking today)
*
* Order rows by selected column.
*/
$query = "SELECT * from orders WHERE 
(order_status = 2 AND date_enter = :today AND time_enter >= :zmiana_start) OR 
(order_status = 2 AND date_enter = :tomorrow AND time_enter < :zmiana_start) OR 
(order_status = 3 AND date_exit = :today AND time_exit >= :zmiana_start) OR 
(order_status = 3 AND date_exit = :tomorrow AND time_exit < :zmiana_start)";
if (isset($_GET['debt'])) $query .= " OR (order_status = 3 AND date_exit < :today) ORDER BY debt";
...
<input type="checkbox" name="debt" id="showDebt" <? if(isset($_GET['debt'])) echo 'checked';?> onclick="showDebt()"> Wyświetlaj opóźnione samochody
    <div id="orders_container">
        <div class="order_row head">
            ...
            <div class="order_col" style="flex-direction: column;"><div class=""><img src="../img/icons/search.png" width="15" onclick="searchIndexInput()" id="searchIndex" alt=""> NR REZERWACJI <a href="?order_by=id">&#9650;</a><a href="?order_by=id&desc=">&#9660;</a><br></div><input type="text" id="searchOrderIndex" style="display: none;" class="search"></div>
            ...
            <div class="order_col">CZAS <a href="?order_by=next_step">&#9650;</a><a href="?order_by=next_step&desc=">&#9660;</a></div>
